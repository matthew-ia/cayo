...
export function resolveDependencies(code, ast, filepath, config) {
  const result = resolveImports(code, filepath, config);
  const cayoComponentPaths = resolveCayoComponents(ast, filepath, config);
  return {
    code: result.code,
...
function resolveCayoComponents(ast, filepath, config) {
  const absoluteBase = path.dirname(filepath);
  const { src: srcPath } = config;
