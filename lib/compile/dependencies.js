...
export function resolveDependencies(rawCode, ast, filepath, config) {
  const result = resolveImports(rawCode, filepath, config);
  const { code, cayoComponentPaths } = resolveCayoComponents(result.code, ast, filepath, config);
  return {
    code,
...
function resolveCayoComponents(code, ast, filepath, config) {
  const absoluteBase = path.dirname(filepath);
  ...
  for (const _path of cayoComponentPaths) {
    ...
    if (_path.startsWith('../') || _path.startsWith('./') || _path.startsWith('/')) {
      ...
      let cayoComponentName = relative
        ...
        .replace('-', '_')
        .replace('.cayo.svelte', '');
      code = code.replace(_path, `${cayoComponentName}-${hash()}`);
    }
  }
