...
export async function compileTemplate(_cayo) {
  ...
  try {
    const { js, css, dependencies } = await build(templatePath, _cayo.config, 'template');
    let outputPath = path.resolve(_cayo.config.cayoPath, `./__cayo/template.js`);
    await fs.outputFile(outputPath, js.code);

    const Template = new Component('template', { js, css }, templatePath, outputPath, dependencies, _cayo.config);
