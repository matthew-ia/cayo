...
export async function compilePages(pageModulePaths, _cayo) {
  ...
  for await (const pagePath of pageModulePaths) {
    try {
      // TODO: remove; no longer need source
      const source = await fs.readFile(pagePath, 'utf8');
      compiledPages.push(
        await compilePage(source, pagePath, _cayo)
      );
    } catch (err) {
      console.log(err);
    }
...
async function compilePage(source, filepath, _cayo) {
  const { layout, stats, config } = _cayo;
