...
function addDependencies(depender, dependencies) {
  ...
  // FIXME: component compilation errors here on depender.dependencies
  // depender.dependencies is not iterable
  let type;
...
export async function handleDependencies(depender, _cayo, handler, config) {
  ...
  // console.log('fuck', _cayo.stats.dependencies[`${depender.type}s`]);
  // depender.dependencies = [..._cayo.stats.dependencies[`${depender.type}s`][depender.path]];
}
