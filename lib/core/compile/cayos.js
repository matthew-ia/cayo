...
export async function compileCayos(components, _cayo, config) {
  ...
  for await (const [name, { src }] of components) {
    try {
      compiledComponents.push(
        await compileCayo(name, src, _cayo, config)
      );
    } catch (err) {
      console.log(err);
    }
...
export async function compileCayo(name, filepath, _cayo, config) {
