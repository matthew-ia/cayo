...
export async function processPage(content, page, _cayo, logger) {
  ...
  for (const el of document.querySelectorAll('[data-cayo-src]')) {
    ...
    if (src !== '') {
      ...
      // If src looks like an explicit import specifier (from component prop), use the resolver.
      // Otherwise treat it as a legacy src-prop value relative to config.components.
      let absoluteSrc;
      if (isExplicitImportSource(src)) {
        try {
          absoluteSrc = await resolveImportSource(src, page.sourcePath, config);
        } catch (err) {
          throw new Error(`Could not resolve Cayo component '${src}': ${err.message}`);
        }
      } else {
      ...
      if (_cayo.stats.cayoComponents[name]) {
        ...
        _cayo.stats.cayoComponents[name].importSource = src;
...
// Returns true for explicit import specifiers injected by the component-prop preprocessor:
//   - Relative paths:       ./foo.svelte  ../bar.svelte
//   - Absolute paths:       /abs/path.svelte
//   - Scoped packages:      @scope/pkg/Foo.svelte
// Returns false for legacy src-prop values (resolved relative to config.components):
//   - Plain filenames:      counter.cayo.svelte
//   - Nested local paths:   nested/counter.svelte
// Note: bare non-scoped package specifiers (e.g. my-pkg/Foo.svelte) are not detectable
// from a plain nested path — use the component prop for external packages.
function isExplicitImportSource(src) {
  return (
    src.startsWith('./') ||
    src.startsWith('../') ||
    src.startsWith('/') ||
