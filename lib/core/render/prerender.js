...
export async function processPage(content, page, _cayo, logger) {
  ...
  // Handle injecting static CSS assets from Cayos (compiled output)
  ...
  if (entryScriptSrc) {
    ...
    // TODO: change this check, and don't use / as prefix, 
    // but maybe nothing or a fake alias like $src/ or %cayo.src%/instead
    if (true) {
      ...
      if (!fs.pathExistsSync(absoluteEntrySrcPath)) {
        ...
        let cayoEntriesPath = path.resolve(config.cayoPath, './__cayo/entries/');
        entry.outputPath = path.resolve(cayoEntriesPath, entryScriptSrc);
        entry.src = absoluteEntrySrcPath;
        _cayo.stats.dependencies.pages[page.sourcePath].add(absoluteEntrySrcPath);
        entryScript.src = path.join('./__cayo/entries/', entryScriptSrc);
      }
      ...
      errors.page.entryNotRelative = {
        title: `Bad entry file`,
        src: page.sourcePath.replace(config.pages, ''),
        message: `Entry file path '${entryScriptSrc}' requires a leading slash and to be relative to the src path.`,
        log: `Bad entry file: '${entryScriptSrc}'. Entry files paths require a leading slash and to be relative to the src path.'`,
      }
    }
