...
import { resolveImportSource } from '../resolver.js';
...
export async function processPage(content, page, _cayo, logger) {
  ...
  for (const el of document.querySelectorAll('[data-cayo-src]')) {
    ...
    if (src !== '') {
      // src may be a JSON-encoded named import: {"from":"pkg","named":"ExportName"}
      let parsedSrc = null;
      try {
        const attempt = JSON.parse(src);
        if (attempt?.from) parsedSrc = attempt;
      } catch {}

      // For ID generation, use a synthetic key that puts the named export as the
      // filename so we get a readable ID like `QuickFilterGroup_abc1234`.
      const idKey = parsedSrc?.named
        ? `${parsedSrc.from}/${parsedSrc.named}.svelte`
        : src;
      const { id, name } = generateCayoComponentId(idKey);
      cayoIds.push(id);
      el.dataset.cayoId = id;
      // Overwrite data-cayo-src with the safe Name_hash ID
      el.dataset.cayoSrc = name;
      ...
      // For JSON-encoded named imports, resolve the `from` specifier.
      // For plain strings, use isExplicitImportSource to decide resolution strategy.
      let absoluteSrc;
      if (parsedSrc) {
        try {
          absoluteSrc = await resolveImportSource(parsedSrc.from, page.sourcePath, config);
        } catch {
          absoluteSrc = parsedSrc.from; // fallback: keep the specifier
        }
      } else {
        try {
          absoluteSrc = await resolveImportSource(src, page.sourcePath, config);
        } catch {
          // Legacy: plain filename used as src prop (e.g. src="counter.cayo.svelte")
          absoluteSrc = path.resolve(config.components, src);
        }
      }
      if (_cayo.stats.cayoComponents[name]) {
        _cayo.stats.cayoComponents[name].src = absoluteSrc;
        _cayo.stats.cayoComponents[name].importSource = parsedSrc?.from ?? src;
        _cayo.stats.cayoComponents[name].named = parsedSrc?.named ?? null;
        _cayo.stats.cayoComponents[name].pages.add(page.sourcePath);
      } else {
        _cayo.stats.cayoComponents[name] = { 
          src: absoluteSrc,
          importSource: parsedSrc?.from ?? src,
          named: parsedSrc?.named ?? null,
  ...
  if (entryScriptSrc) {
    ...
    if (cayoIds.length > 0) {
      ...
      // Remove the entry script tag — no entry file means no index.js will be compiled,
      // and leaving the tag in causes Rollup to fail with an unresolved import error.
      entryScript?.remove();
    } else {
