    ...
    if (userEntryFile.startsWith('/')) {
      ...
      if (!fs.pathExistsSync(entryFilePath)) {
        ...
        try {
          ...
          // TODO: this isn't a great check, but it should mostly work
          if (!entrySource.includes(`./cayo-runtime.js`) && cayoIds.length > 0) {
            let relativePath = entryFilePath.replace(config.projectRoot, '');
            errors.page.entryNeedsCayoRuntime = {
              title: `Bad entry file`,
              src: page.sourcePath.replace(config.pages, ''),
              message: `Entry file '${relativePath}' does not import the default export of ./cayo-runtime.js. This is required to render cayos.`,
              log: `Bad entry file: '${userEntryFile}'. Entry file does not import the default export of ./cayo-runtime.js. This is required to render cayos.`,
            }
          }
