...
export async function processPage(content, page, _cayo, logger) {
  ...
  for (const el of document.querySelectorAll('[data-cayo-src]')) {
    ...
    if (src !== '') {
      ...
      if (el.dataset.cayoWarn) {
        warnings[id] = el.dataset.cayoWarn;
  ...
  // Append any console warnings for Cayos
  if (Object.keys(warnings).length > 0) {
    const script = document.createElement('script');
    script.innerHTML = '/* Cayo instance warnings to be show in the runtime console */\n'
    for (const key in warnings) {
      script.innerHTML += `console.warn(\`Warning: Cayo component may have runtime issues.\n\n${warnings[key]}\nCayo ID: ${key}:\`, document.querySelector('[data-cayo-id="${key}"]'));\n`;
    }
