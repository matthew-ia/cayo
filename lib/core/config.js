...
export async function validateConfig(userConfig) {
  const ConfigSchema = z.object({
    ...
    // Currently still used for <Cayo src="..." /> resolution for backward compat., which is deprecated syntax.
    components: z
      .string()
      .default('./components'),
    publicDir: z
...
async function handlePaths(config, input, base) {
  ...
  const pathsToCheck = { projectRoot, src, pages, templatePath, publicDir };
