...
export default function preprocess(sourcePath, srcDirPath, cayoDirPath) {
  ...
  const script = (input) => {
    transformScript(input, sourcePath, srcDirPath, cayoDirPath); 
  }

  const markup = (input) => {
    transformMarkup(input, sourcePath, srcDirPath, cayoDirPath);
  }
...
function transformScript(input, sourcePath, srcDirPath, cayoDirPath) {
...
function markupPreprocessor(input, sourcePath, srcDirPath, cayoDirPath) {
  ...
  content = cayoComponentNames(content, filename);
