...
export async function getEntryDependencies(entry, page, _cayo) {
  ...
  let absoluteDeps = localDeps.map(d => path.resolve(path.dirname(entry.src), d));
  let srcRelativeLocalDeps = localDeps.map((d, i) => {
    let srcRelativePath = absoluteDeps[i].replace(config.src, '');
    // From .cayo/__cayo/entries/<outputPathDir>, to src path
    let pathToSrc = path.relative(path.dirname(entry.outputPath), config.src);
    return path.join(pathToSrc, srcRelativePath);
  });
