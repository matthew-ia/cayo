...
export async function build(input, config, type = 'page') {
  ...
  const compilerOptions = {
    generate: (type === 'page') ? 'ssr' : 'dom', 
    dev: false, // probably want this
    hydratable: (type === 'page') ? false : true, // this is default, may need to be changed just fo Cayo components
  ...
  try {
    ...
    // if (input.includes('index')) console.log(input, output.code);
