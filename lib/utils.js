...
export function resolveImports(code, config) {
  ...
  imports.forEach(_path => {
    // TODO: this won't get aliased stuff, but that may work anyway
    if (_path.startsWith('../') || _path.startsWith('./') || _path.startsWith('/')) {
      console.log('original', _path);
  ...
  // code.match(/^import (\"?\'?[/s/S]+\.[/s/S]\"?\'?)/g)
  let rePath = /([\'\"]{1,1}?[\w\d\.\/]+)[\'\"]{1,1}?/g;
  let reImport = /(?:import)([\s\S][^\n]+)\n/gm
  // let reImport = /^(import)/g
  let m = code.match(reImport);
  m.forEach((str) => {
    console.log('import', str);
    let m2 = str.match(rePath);
    console.log('path', m2);
  })
  console.log(m);
}
