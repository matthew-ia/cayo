...
export function resolveImports(code, config) {
  const { src: srcPath, pages: pagesPath } = config;
  const imports = precinct(code);
  const deps = [];
  let newCode = code;
  imports.forEach(_path => {
    ...
    if (_path.startsWith('../') || _path.startsWith('./') || _path.startsWith('/')) {
      let resolved = path.resolve(pagesPath, _path);
...
function addDependencies(depender, dependencies) {
  if (!dependencies[depender.path]) {
    dependencies[depender.path] = depender.dependencies;
  } else {
    dependencies[depender.path] = [...dependencies[depender.path], ...depender.dependencies];
  }
}
