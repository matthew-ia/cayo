...
import { loadConfig, checkParsedConfig } from '../core/config.js';
...
async function run(command) {
  ...
  try {
    ...
    checkParsedConfig(cayoConfig, logger);
  } catch (err) {
    logger.error(err);
    process.exit(1);
  }
  ...
  try {
    ...
    const Layout = await compile.layout(_cayo.config);
    ...
    logger.error(err.message, { timestamp: true, clear: false, });
    process.exit(1);
  }
}
