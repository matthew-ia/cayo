...
async function run(command) {
  ...
  try {
    ...
    checkConfigPaths(cayoConfig, logger);
    ...
    for (const [key, page] of _cayo.pages) {
      ...
      writePageFiles(page, cayoConfig.cayoPath, cayoConfig);
    }

    await compile.components(null, _cayo, cayoConfig);

    // for (const name in _cayo.stats.cayoComponents) {
    //   // TODO: this bundle stuff needs to go within
    //   const { src } = _cayo.stats.cayoComponents[name];
    //   const component = buildBundle(src, cayoConfig);
    //   writeComponentFile(
    //     { ...component, name }, 
    //     src,
    //     cayoConfig.cayoPath
