...
async function handleComponent(filepath, _cayo, config) {
  ...
  console.log('handling', filepath, _cayo.stats.dependencies.pages)
  await compile.components([filepath], _cayo, config).then(()=>{
    for (const [pagePath, pageDeps] of Object.entries(stats.dependencies.pages)) {
      console.log(pagePath);
      if (pageDeps.has(filepath)) {
        // NOTE: we DO actually need to recompile the page, not just rerender it
...
async function handlePage(filepath, _cayo, config) {
  ...
  const _page = pages.get(filepath);
  await _page.writeModule();
  await _page.loadModule();
  await _page.render(false, stats.cayoComponents);
  writePageFiles(_page, config.cayoPath, config);
}
