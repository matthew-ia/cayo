...
export async function compileComponents(modulePaths, config) {
  const { pages } = config;
  const compiledComponents = [];

  if (modulePaths === null) {
    modulePaths = await getModulePaths(pages);
  }

  modulePaths.forEach(async (modulePath) => {
    try {
      const source = await fs.readFile(modulePath, 'utf8');
      compiledComponents.push(await compileComponent(source, modulePath, config));
    } catch (err) {
      console.log(err);
    }
  });
...
export async function compileComponent(source, filepath, config) {
  ...
  try {
    ...
    const PageComponent = (await import(outputPath)).default;
